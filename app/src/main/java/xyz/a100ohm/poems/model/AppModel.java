package xyz.a100ohm.poems.model;
...
import xyz.a100ohm.poems.model.beans.localdb.DBPoetryTranslate;
import xyz.a100ohm.poems.model.modelinterface.ModelEveryDayInterface;
 ...
 */
public class AppModel implements ModelEveryDayInterface {
     ...
     * 请求获取诗句的回调接口
     */
    public interface RequestPoetrySentenceCallBack {
        void onfoundPoetrySentence(DBPoetrySentence[] sentences);
    }
    /**
     * {@inheritDoc}
     * @param num 诗句数量
    ...
    @Override
    public void requestPoetrySentence(final int num, final RequestPoetrySentenceCallBack callBack) {
        L.d("申请本地获取今日诗词");
        realm.executeTransactionAsync(new Realm.Transaction() {
            @Override
            public void execute(Realm realm) {
                ...
                for(int i = 0; i < num; i++) {//生成一个随机数
                    ...
                    else {//不知道为什么查不出结果，那就先返回默认的吧
                        returnDefault(num, callBack);
                        return;
                    }
                }
                callBack.onfoundPoetrySentence(sentences);
            }
        });
    }

    private void returnDefault(int num, RequestPoetrySentenceCallBack callBack){
        if(num == 3) {
            DBPoetrySentence[] sentences = new DBPoetrySentence[num];
            ...
            callBack.onfoundPoetrySentence(new DBPoetrySentence[]{sentence});
        }
    }
